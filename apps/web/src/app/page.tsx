...
import { RegisterWithCredentialsDocument, CompaniesDocument } from '@autospace/network/src/gql/generated'

export default function Home() {

  const [, { data: regData }] = useMutation(RegisterWithCredentialsDocument)
  const { data, loading } = useQuery(CompaniesDocument // token need to be provide
  ...
  return <main>
    ...
    <div>
      {data?.companies.map((company) => (
        <div
          className='p-4 bg-gray-100 rounded m-4'
          key={company.id}>{company.displayName}</div>
      ))}
    </div>
  </main>
}
